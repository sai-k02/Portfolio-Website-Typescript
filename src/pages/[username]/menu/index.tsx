...
// define Menu
const Menu: NextPage = (SITE_USER_DATA: { _id: string }) => {
    // DEFINE TAB CONTENT
    const [tabContent, setTabContent] = useState(
        <Profile data={SITE_USER_DATA}></Profile>
    );
    ...
    // DO NOT SHOW CONTENT IF USER IS NOT LOGGED IN && IF USER IS NOT THE SAME AS THE MENU PAGE
    if (status === "loading") {
        return (
            <>
    ...
    if (session && session.login === SITE_USER_NAME) {
        return (
            <>
                <HeaderMain></HeaderMain>
                <Divider></Divider>
                <PageHeader
                    ...
                    footer={
                        <Tabs defaultActiveKey="1" onChange={onChange}>
                            <TabPane tab={<Title level={3}>Profile</Title>} key="1">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                            <TabPane tab={<Title level={3}>Pro-folio</Title>} key="2">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                            <TabPane tab={<Title level={3}>Experiences</Title>} key="3">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                            <TabPane tab={<Title level={3}>Projects</Title>} key="4">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                            <TabPane tab={<Title level={3}>Technologies</Title>} key="5">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                            <TabPane tab={<Title level={3}>Settings</Title>} key="6">
                                {" "}
                                {tabContent}{" "}
                            </TabPane>
                        </Tabs>
...
export const getServerSideProps: GetServerSideProps = async (
    ...
    let raw_response = await axios(config).catch((e: any) => {
        console.log(e);
    });
    if (raw_response) {
